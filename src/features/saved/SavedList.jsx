  ...
  if (!saved.length) {
    return (
      <div className={styles.empty}>
        ...
        <h2 className={styles.emptyTitle}>No saved flights</h2>
        <p className={styles.emptyText}>
          Save flights from the search results to compare them here.
        </p>
        <Link to="/">
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>Saved flights</h1>
          <p className={styles.subtitle}>
            {saved.length} {saved.length === 1 ? 'flight' : 'flights'} saved
          </p>
        </div>
          ...
          {saved.map(({ id, offer, savedAt }) => (
            ...
            >
              ...
              <div className={styles.cardHeader}>
                ...
                >
                  {offer.dealScore}
                  <span className={styles.scoreLabel}>{dealScoreLabel(offer.dealScore)}</span>
                </div>
              </div>
              ...
              <div className={styles.details}>
                <div className={styles.detail}>
                  <span className={styles.detailLabel}>Departs</span>
              ...
              <div className={styles.cardFooter}>
                <div className={styles.badges}>
                  <Badge variant={offer.stops === 0 ? 'success' : 'default'}>
                    {formatStops(offer.stops)}
                  </Badge>
                  {offer.co2Kg > 0 && <Badge variant="default">🌱 {formatCO2(offer.co2Kg)}</Badge>}
